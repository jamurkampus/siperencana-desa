...
export default async function DashboardPage() {
  ...
  const statCards = [
    { label: "Total Kegiatan", value: data.totalKegiatan, icon: FileText, color: "bg-blue-50 text-blue-600", change: `TA ${data.tahunAktif?.tahun ?? "-"}` },
    { label: "Total Anggaran", value: formatRupiah(data.totalAnggaran), icon: TrendingUp, color: "bg-emerald-50 text-emerald-600", change: "Seluruh kegiatan" },
    { label: "Sedang Berjalan", value: data.kegiatanBerjalan, icon: PlayCircle, color: "bg-amber-50 text-amber-600", change: "Kegiatan aktif" },
    { label: "Selesai", value: data.kegiatanSelesai, icon: CheckCircle, color: "bg-purple-50 text-purple-600", change: "Kegiatan tuntas" },
  ];
      ...
      <div className="bg-gradient-to-br from-primary-700 to-primary-900 rounded-2xl p-6 text-white">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-primary-200 text-sm mt-1">Kaur Perencanaan · Desa {data.desa?.namaDesa}, Kec. {data.desa?.kecamatan}</p>
          </div>
              ...
              {data.recentKegiatan.map((k) => (
                <Link key={k.id} href={`/kegiatan/${k.id}`} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{labelBidang[k.bidang as keyof typeof labelBidang] ?? k.bidang} · TA {k.tahunAnggaran.tahun}</p>
                  </div>
                  ...
                  return (
                    <div key={b.bidang}>
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="text-gray-600 font-medium truncate pr-2">{(labelBidang[b.bidang as keyof typeof labelBidang] ?? b.bidang).split(" ").slice(0, 2).join(" ")}</span>
